...
| 文件 | 来源 |
|------|------|
| `skill_context/人物锚点.md` | 从描述中识别角色 → 填充角色列表（弧线类型/入口状态） |
| `skill_context/技能锚点.md` | 从描述中识别技能 → 填充技能锚点表 |
| `skill_context/宝物锚点.md` | 从描述中识别宝物 → 填充宝物锚点表 |
| `skill_context/写作纲领.md` | 填充题材+**分类**+**命名风格**+质感规则；**禁区**同步到全局约束章节 |
...
- 所有结构化文件（人物锚点/技能锚点/宝物锚点/写作纲领）由 Agent 从 `user_input.md` 中**提取信息并生成**
...
用户提到的禁区 → 写入 `skill_context/写作纲领.md` 的「全局约束」章节，后续所有阶段都读该文件。
