...
description: 幻想小说全写 Pipeline——ignite → Spine → Scene Design（逐 Scene） + QC → 用户手动触发写作 → Scene 正文过程稿 → 全稿QC（只RECORD） → 生成锚点更新草案 → 用户确认后生成最终稿并更新全局锚点 → Pipeline结束。
type: protocol
pattern: sequential
category: creative
...
| **③ Scene Design** | `开始设计` 等 | scene{X}-design.md（全量） | scene-design-qc → 用户手动触发写作 |
| **④ 写作** | `开始写` 等 | ep{N}-scene{X}.md + anchor-update-draft.md | write-qc → 用户确认收尾 / 下次 ignite 前强制结清 → **Pipeline结束** |
...
| Scene Design → scene-design-qc | 允许 |
| scene-design-qc → Write | 不允许 |
| Write → write-qc | 允许 |
...
EP1 写作: fantasy-scene-write（按 Scene 输出 → ep1/workspace/ep1-scene{X}.md）
    ↓ 自动进入全稿 QC（只 RECORD，输出 ep1/workspace/write-qc.md）
    ↓ 生成 ep1/workspace/anchor-update-draft.md（按对象级变更单元组织）
    ↓ 用户确认 Scene 过程稿 + QC + 锚点草案后，才生成 ep1/ep1.md
...
先询问用户题材：`fantasy`（奇幻） / `mecha`（机甲），写入 `skill_context/genre.md`。
...
- `<project_root>/skill_context/机体设定集/`
- `<project_root>/skill_context/科技设定集/`
- `<project_root>/skill_context/阵营设定集/`
- `<project_root>/skill_context/舰艇设定集/`（可选）
...
**题材提示：** 如 `genre.md` 为 `mecha`，提醒用户可在输入中包含机体/科技/阵营相关的初始构想（可选）。

### Step 3: Agent 自动生成锚点文件

**3.1** 读取 `skill_context/genre.md` 确定题材。读取模板文件（`references/project-skeleton/skill_context/` 下所有锚点模板）。
...
| 题材 | 附加文件 |
|------|---------|
| fantasy | `skill_context/技能锚点.md`（从描述中识别技能） |
| fantasy | `skill_context/宝物锚点.md`（从描述中识别宝物） |
| mecha | `skill_context/机体锚点.md`（从描述中识别机体型号/名称） |
| mecha | `skill_context/科技锚点.md`（从描述中识别关键科技） |
| mecha | `skill_context/阵营锚点.md`（从描述中识别势力） |

> **设定集（人物/技能/宝物/机体/科技/阵营）不在 Ignite 生成**，由 Spine Step 6 负责。
...
- 完成后执行 Scene Design QC（只 RECORD，不 FAIL），输出 `scene-design-qc.md`，并额外检查 design 是否达到 **write-ready** 粒度
...
- 按 Scene 输出正文过程稿：`ep{N}/workspace/ep{N}-scene1.md` 到 `ep{N}-scene{S}.md`
...
- 最终稿 `ep{N}/ep{N}.md` 只在用户确认收尾后生成；`ep{N}/ep{N}.md` 是唯一最终稿路径
...
- 用 `user_input-template.md`（EP1）或 `next-ep-user_input-template.md`（EP{N+1}）里的自然语言问题引导用户填写
- 可选项标注「如果你愿意多说一点」
...
- 参考：
  - `references/project-skeleton/user_input-template.md`
  - `references/project-skeleton/next-ep-user_input-template.md`
...
- spine-qc / scene-design-qc / write-qc 完成后，才允许出现用户检查或阶段暂停
...
- `../adapters/opencode/orchestration.md` — opencode 适配层说明
- `../adapters/hermes/orchestration.md` — Hermes 适配层说明
- `../adapters/codex/orchestration.md` — Codex 适配层说明
- `../adapters/codex/multi-agent.md` — Codex 多 Agent 写集合约束
...
读取 `skill_context/genre.md` 确定题材。读取 `skill_context/EP锚点.md`、`人物锚点.md`，了解：
- 已完成哪些 EP，各 EP 出口状态
- 主角当前状态

题材特定读取：
- `fantasy`：`技能锚点.md`、`宝物锚点.md`
- `mecha`：`机体锚点.md`、`科技锚点.md`、`阵营锚点.md`
...
- `<project_root>/ep{N}/user_input.md`（由 `references/project-skeleton/next-ep-user_input-template.md` 生成）
