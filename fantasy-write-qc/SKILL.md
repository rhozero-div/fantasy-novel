...
- 修仙：古意语言/顿悟节奏/宗门规矩融入
- 武侠：江湖气白话/写意动作/克制情感
- 机甲：军事科幻语感/技术术语自然嵌入/重质量感描写/驾驶舱内视角主导
...
**题材扩展 — 机甲武戏（`mecha`）：**
- 机体参数一致性：机体状态变化（能量/装甲/武装）是否与设计一致 → 记录偏离
- 武装使用合理：是否出现了机体未配置的武器 → 记录违规使用
- 环境反馈：战斗环境（宇宙/大气/城市等）是否在描写中有体现 → 记录缺失
- 重量感：机体动作是否有巨大质量感（地面震动、惯性等）→ 记录描写缺失
- 驾驶舱 POV 稳定性：是否保持了驾驶舱视角 → 记录 POV 跳脱
...
- 内容包括（题材特定）：
  - fantasy：`skill_context/技能锚点.md` 的拟更新项、`skill_context/宝物锚点.md` 的拟更新项
  - mecha：`skill_context/机体锚点.md` 的拟更新项、`skill_context/科技锚点.md` 的拟更新项、`skill_context/阵营锚点.md` 的拟更新项
  - **设定集履历**的拟追加项：`skill_context/人物设定集/`、`技能设定集/`、`宝物设定集/`（mecha 题材追加 `机体设定集/`、`科技设定集/`、`阵营设定集/`）
